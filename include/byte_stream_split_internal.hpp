// From the Apache Arrow Library, modified by Michael Bernardi.
...
{
template <typename T> inline std::enable_if_t<std::is_trivially_copyable_v<T>, T> SafeLoadAs(const uint8_t *unaligned)
{
...
template <typename T> inline std::enable_if_t<std::is_trivially_copyable_v<T>, T> SafeLoad(const T *unaligned)
{
    std::remove_const_t<T> ret;
    std::memcpy(&ret, unaligned, sizeof(T));
    return ret;
...
inline std::enable_if_t<std::is_trivially_copyable_v<T> && std::is_trivially_copyable_v<U> && sizeof(T) == sizeof(U), U>
SafeCopy(T value)
{
...
template <typename T> inline std::enable_if_t<std::is_trivially_copyable_v<T>, void> SafeStore(void *unaligned, T value)
{
...
template <typename T> void ByteStreamSplitDecodeSse2(const uint8_t *data, int64_t num_values, int64_t stride, T *out)
{
    ...
    for (int64_t i = 0; i < num_blocks; ++i)
    {
        ...
        {
            stage[0][j] = _mm_loadu_si128(reinterpret_cast<const __m128i *>(&data[i * sizeof(__m128i) + j * stride]));
        }
        for (size_t step = 0; step < kNumStreamsLog2; ++step)
        {
            for (size_t j = 0; j < kNumStreamsHalf; ++j)
            {
                stage[step + 1U][j * 2] = _mm_unpacklo_epi8(stage[step][j], stage[step][kNumStreamsHalf + j]);
                stage[step + 1U][j * 2 + 1U] = _mm_unpackhi_epi8(stage[step][j], stage[step][kNumStreamsHalf + j]);
            }
        }
        for (size_t j = 0; j < kNumStreams; ++j)
        {
            _mm_storeu_si128(reinterpret_cast<__m128i *>(&output_data[(i * kNumStreams + j) * sizeof(__m128i)]),
...
template <typename T>
void ByteStreamSplitEncodeSse2(const uint8_t *raw_values, const size_t num_values, uint8_t *output_buffer_raw)
{
    constexpr size_t kNumStreams = sizeof(T);
    static_assert(kNumStreams == 4U || kNumStreams == 8U, "Invalid number of streams.");
    ...
    {
        output_buffer_streams[i] = reinterpret_cast<__m128i *>(&output_buffer_raw[num_values * i]);
    }
    ...
    for (size_t block_index = 0; block_index < num_blocks; ++block_index)
    {
        ...
        for (size_t stage_lvl = 0; stage_lvl < 2U; ++stage_lvl)
        {
            ...
            {
                stage[stage_lvl + 1][i * 2] = _mm_unpacklo_epi8(stage[stage_lvl][i * 2], stage[stage_lvl][i * 2 + 1]);
...
template <typename T> void ByteStreamSplitDecodeAvx2(const uint8_t *data, int64_t num_values, int64_t stride, T *out)
{
    constexpr size_t kNumStreams = sizeof(T);
    static_assert(kNumStreams == 4U || kNumStreams == 8U, "Invalid number of streams.");
    constexpr size_t kNumStreamsLog2 = (kNumStreams == 8U ? 3U : 2U);
    ...
    for (int64_t i = 0; i < num_blocks; ++i)
    {
        ...
        {
            stage[0][j] =
                _mm256_loadu_si256(reinterpret_cast<const __m256i *>(&data[i * sizeof(__m256i) + j * stride]));
        }
        ...
        for (size_t step = 0; step < kNumStreamsLog2; ++step)
        {
            for (size_t j = 0; j < kNumStreamsHalf; ++j)
            {
                stage[step + 1U][j * 2] = _mm256_unpacklo_epi8(stage[step][j], stage[step][kNumStreamsHalf + j]);
                stage[step + 1U][j * 2 + 1U] = _mm256_unpackhi_epi8(stage[step][j], stage[step][kNumStreamsHalf + j]);
            }
        }
        ...
        for (size_t j = 0; j < kNumStreams; ++j)
        {
            _mm256_storeu_si256(reinterpret_cast<__m256i *>(&output_data[(i * kNumStreams + j) * sizeof(__m256i)]),
                                final_result[j]);
        }
    }
}
...
template <typename T>
void ByteStreamSplitEncodeAvx2(const uint8_t *raw_values, const size_t num_values, uint8_t *output_buffer_raw)
{
    constexpr size_t kNumStreams = sizeof(T);
    static_assert(kNumStreams == 4U || kNumStreams == 8U, "Invalid number of streams.");
...
template <typename T> void ByteStreamSplitDecodeAvx512(const uint8_t *data, int64_t num_values, int64_t stride, T *out)
{
    constexpr size_t kNumStreams = sizeof(T);
    static_assert(kNumStreams == 4U || kNumStreams == 8U, "Invalid number of streams.");
    constexpr size_t kNumStreamsLog2 = (kNumStreams == 8U ? 3U : 2U);
    ...
    for (int64_t i = 0; i < num_blocks; ++i)
    {
        ...
        {
            stage[0][j] =
                _mm512_loadu_si512(reinterpret_cast<const __m512i *>(&data[i * sizeof(__m512i) + j * stride]));
        }
        ...
        for (size_t step = 0; step < kNumStreamsLog2; ++step)
        {
            for (size_t j = 0; j < kNumStreamsHalf; ++j)
            {
                stage[step + 1U][j * 2] = _mm512_unpacklo_epi8(stage[step][j], stage[step][kNumStreamsHalf + j]);
                stage[step + 1U][j * 2 + 1U] = _mm512_unpackhi_epi8(stage[step][j], stage[step][kNumStreamsHalf + j]);
            }
        }
        ...
        if constexpr (kNumStreams == 8U)
        {
            ...
            shuffle[0] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][0], stage[kNumStreamsLog2][1], 0b01000100);
            shuffle[1] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][2], stage[kNumStreamsLog2][3], 0b01000100);
            shuffle[2] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][4], stage[kNumStreamsLog2][5], 0b01000100);
            shuffle[3] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][6], stage[kNumStreamsLog2][7], 0b01000100);
            shuffle[4] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][0], stage[kNumStreamsLog2][1], 0b11101110);
            shuffle[5] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][2], stage[kNumStreamsLog2][3], 0b11101110);
            shuffle[6] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][4], stage[kNumStreamsLog2][5], 0b11101110);
            shuffle[7] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][6], stage[kNumStreamsLog2][7], 0b11101110);
        ...
        else
        {
            ...
            shuffle[0] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][0], stage[kNumStreamsLog2][1], 0b01000100);
            shuffle[1] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][2], stage[kNumStreamsLog2][3], 0b01000100);
            shuffle[2] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][0], stage[kNumStreamsLog2][1], 0b11101110);
            shuffle[3] = _mm512_shuffle_i32x4(stage[kNumStreamsLog2][2], stage[kNumStreamsLog2][3], 0b11101110);
        ...
        for (size_t j = 0; j < kNumStreams; ++j)
        {
            _mm512_storeu_si512(reinterpret_cast<__m512i *>(&output_data[(i * kNumStreams + j) * sizeof(__m512i)]),
                                final_result[j]);
        }
    }
}
...
template <typename T>
void ByteStreamSplitEncodeAvx512(const uint8_t *raw_values, const size_t num_values, uint8_t *output_buffer_raw)
{
    constexpr size_t kNumStreams = sizeof(T);
    static_assert(kNumStreams == 4U || kNumStreams == 8U, "Invalid number of streams.");
    ...
    for (size_t i = 0; i < kNumStreams; ++i)
    {
        output_buffer_streams[i] = reinterpret_cast<__m512i *>(&output_buffer_raw[num_values * i]);
    }
    ...
    if constexpr (kNumStreams == 8U)
    {
        ...
        permutex_mask = _mm512_set_epi32(0x001F0017, 0x000F0007, 0x001E0016, 0x000E0006, 0x001D0015, 0x000D0005,
                                         0x001C0014, 0x000C0004, 0x001B0013, 0x000B0003, 0x001A0012, 0x000A0002,
                                         0x00190011, 0x00090001, 0x00180010, 0x00080000);
    }
    else
    {
        permutex_mask = _mm512_set_epi32(0x0F, 0x0B, 0x07, 0x03, 0x0E, 0x0A, 0x06, 0x02, 0x0D, 0x09, 0x05, 0x01, 0x0C,
                                         0x08, 0x04, 0x00);
    }
    ...
    for (size_t block_index = 0; block_index < num_blocks; ++block_index)
    {
        ...
        for (size_t unpack_lvl = 0; unpack_lvl < KNumUnpack; ++unpack_lvl)
        {
            for (size_t i = 0; i < kNumStreams / 2U; ++i)
            {
                unpack[unpack_lvl + 1][i * 2] =
                    _mm512_unpacklo_epi8(unpack[unpack_lvl][i * 2], unpack[unpack_lvl][i * 2 + 1]);
                unpack[unpack_lvl + 1][i * 2 + 1] =
                    _mm512_unpackhi_epi8(unpack[unpack_lvl][i * 2], unpack[unpack_lvl][i * 2 + 1]);
            }
        }
...
template <typename T>
void inline ByteStreamSplitDecodeSimd(const uint8_t *data, int64_t num_values, int64_t stride, T *out)
{
#if defined(ARROW_HAVE_AVX512)
...
{
#if defined(ARROW_HAVE_AVX512)
    return ByteStreamSplitEncodeAvx512<T>(raw_values, static_cast<size_t>(num_values), output_buffer_raw);
#elif defined(ARROW_HAVE_AVX2)
    return ByteStreamSplitEncodeAvx2<T>(raw_values, static_cast<size_t>(num_values), output_buffer_raw);
#elif defined(ARROW_HAVE_SSE4_2)
    return ByteStreamSplitEncodeSse2<T>(raw_values, static_cast<size_t>(num_values), output_buffer_raw);
#else
...
template <typename T>
void ByteStreamSplitEncodeScalar(const uint8_t *raw_values, const int64_t num_values, uint8_t *output_buffer_raw)
{
...
template <typename T> void ByteStreamSplitDecodeScalar(const uint8_t *data, int64_t num_values, int64_t stride, T *out)
{
    constexpr int kNumStreams = static_cast<int>(sizeof(T));
    ...
    DoMergeStreams(src_streams.data(), kNumStreams, num_values, reinterpret_cast<uint8_t *>(out));
}
...
template <typename T> void inline ByteStreamSplitDecode(const uint8_t *data, int64_t num_values, int64_t stride, T *out)
{
#if defined(ARROW_HAVE_SIMD_SPLIT)
